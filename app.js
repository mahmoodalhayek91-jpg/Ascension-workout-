...
const routines = {
  A: { title: 'Foundation', exercises: [
    { name: 'Goblet Squat', baseWeight: 12, group: 'lower' },
    { name: 'Push-Up', baseWeight: 0, group: 'body' },
    { name: 'One-Arm Row', baseWeight: 10, group: 'upper' },
    { name: 'Romanian Deadlift', baseWeight: 16, group: 'lower' },
    { name: 'Plank', seconds: 25, group: 'core' }
  ]},
  B: { title: 'Vanguard', exercises: [
    { name: 'Reverse Lunge', baseWeight: 8, group: 'lower' },
    { name: 'Shoulder Press', baseWeight: 6, group: 'upper' },
    { name: 'Lat Pulldown', baseWeight: 20, group: 'upper' },
    { name: 'Hip Thrust', baseWeight: 20, group: 'lower' },
  ...
  F: { title: 'Bodyweight Trial', exercises: [
    { name: 'Bodyweight Squat', baseWeight: 0, group: 'body' },
    { name: 'Incline Push-Up', baseWeight: 0, group: 'body' },
    { name: 'Inverted Row', baseWeight: 0, group: 'body' },
    { name: 'Glute Bridge', baseWeight: 0, group: 'body' },
    { name: 'Mountain Climber', seconds: 25, group: 'core' }
  ]}
};

const initial = { name: 'Mahmoud', xp: 0, streak: 0, lastWorkout: null, workoutCount: 0, routine: 'A', history: [], bests: {}, draft: null, recoveryDate: null, recoveryCount: 0 };
let state = load();
let page = 'home';
...
function home(){
  ...
  <article class="quest"><div class="quest-icon">⚔</div><div class="quest-body"><h3>Strength Quest ${next} · ${routines[next].title}</h3><p>${routines[next].exercises.map(x=>x.name).slice(0,3).join(' · ')}</p></div><div class="reward">+100 XP</div></article>
...
function ensureDraft(){
  if(!state.draft || state.draft.routine!==state.routine || state.draft.version!==2){
    state.draft={version:2,routine:state.routine,level:level(),exercises:routines[state.routine].exercises.map(ex=>({...ex,sets:[false,false,false]}))}; save();
  }
}
function workout(){
  ensureDraft();
...
function finish(){
  ...
  state.history.unshift({date:now.toISOString(),routine:state.routine}); state.lastWorkout=now.toISOString(); state.workoutCount++; state.xp+=100;
...
function importData(e){const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{state={...initial,...JSON.parse(r.result),draft:null};if(!routineOrder.includes(state.routine))state.routine='A';save();page='home';render();toast('Backup restored')}catch{alert('That backup file could not be read.')}};r.readAsText(f);}
