...
function applyLanguage(){const arabic=state.language==='ar';document.documentElement.lang=arabic?'ar':'en';document.documentElement.dir=arabic?'rtl':'ltr';document.documentElement.dataset.language=arabic?'ar':'en';}
...
function settings(){
  ...
  const languages=[['en','English','English interface'],['ar','العربية','واجهة عربية كاملة']].map(([id,label,detail])=>`<button class="mode-option ${state.language===id?'active':''}" data-language="${id}"><b>${label}</b><span>${detail}</span>${state.language===id?'<i>✓</i>':''}</button>`).join('');
...
function bind(){
  ...
  document.querySelectorAll('[data-language]').forEach(b=>b.onclick=()=>{state.language=b.dataset.language;save();render();toast(state.language==='ar'?'تم تغيير اللغة إلى العربية':'Language changed to English')});
